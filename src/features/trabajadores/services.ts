...
export async function getTrabajadores(sucursalId?: number): Promise<ApiResponse> {
  try {
    let query = `
      ...
      (SELECT COALESCE(SUM(VL_MONTO), 0) FROM KS_VALES WHERE TR_IDTRABAJADOR_FK = t.TR_IDTRABAJADOR_PK AND VL_ESTADO != 'ANULADO') as total_vales,
      (SELECT COUNT(*) FROM KS_VALES WHERE TR_IDTRABAJADOR_FK = t.TR_IDTRABAJADOR_PK AND VL_ESTADO = 'PENDIENTE') as vales_pendientes
