...
import { previewLiquidacionIndividual, liquidarTrabajadorIndividual, getWorkerLastSettlementDate } from '@/features/nomina/services'
import { VolantePago } from '@/components/nomina/volante-pago'
import { toast } from '@/lib/toast-helper'

interface IndividualSettlementModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
  workerId: number
...
export function IndividualSettlementModal({
  isOpen,
  onClose,
  onSuccess,
  workerId,
  workerName,
  workerRole
}: IndividualSettlementModalProps) {
  ...
  const handlePreview = async () => {
    ...
    try {
      const res = await previewLiquidacionIndividual(workerId, new Date(endDate + 'T12:00:00'), basePay)
  ...
  const handleConfirmLiquidation = async () => {
    ...
    try {
      const res = await liquidarTrabajadorIndividual(workerId, new Date(endDate + 'T12:00:00'), basePay)
      if (res.success) {
        toast.success('TRABAJADOR LIQUIDADO', res.message || 'La liquidación ha sido confirmada con éxito.')
        onSuccess()
        onClose()
      } else {
        toast.error('ERROR EN LIQUIDACIÓN', res.error || 'Ocurrió un error al procesar la liquidación.')
      }
    } catch (err) {
      console.error(err)
      ...
      <DialogContent className="max-w-[95vw] sm:max-w-[650px] lg:max-w-[800px] max-h-[90vh] overflow-y-auto border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 rounded-3xl p-0 shadow-2xl animate-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-lg font-bold tracking-tight text-slate-950 dark:text-slate-50">
                Liquidación Individual de "{workerName}"
              </DialogTitle>
              <DialogDescription className="text-xs text-slate-400 font-medium pt-1">
                Genera un volante de pago individual parcial. El colaborador continuará activo.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="fecha-liquidacion" className="text-[10px] font-bold uppercase text-slate-400 tracking-wider flex items-center gap-1.5">
                <Calendar className="size-3.5" /> Liquidar Hasta
              </Label>
              <Input
            ...
            {/* Removed motivo for individual settlement */}
          </div>
          ...
          {previewData && (
            <div className="border border-emerald-100/80 dark:border-emerald-950/30 bg-emerald-50/30 dark:bg-emerald-950/10 rounded-2xl p-4 space-y-4">
              <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-500 font-bold text-xs uppercase tracking-wider">
                <AlertCircle className="size-4 animate-bounce" /> Confirmación de Liquidación Individual
              </div>
              <p className="text-[11px] text-slate-600 dark:text-slate-300 font-medium">
                Al confirmar, se descontarán <span className="font-bold">solo las cuotas vencidas</span> de las deudas del trabajador y la nómina individual quedará pagada. <span className="font-bold text-emerald-600 dark:text-emerald-400">"{workerName}"</span> continuará <span className="font-bold">ACTIVO</span> en el sistema.
              </p>
        ...
        <DialogFooter className="p-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800/80 gap-2 flex flex-col sm:flex-row-reverse">
          <Button
            ...
            className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl h-10 px-6 shadow-md shadow-emerald-600/10 text-xs uppercase tracking-wider flex-1 sm:flex-none min-w-[150px] transition-all hover:scale-[1.01] active:scale-[0.99] border-none"
          >
